...
class Settings(BaseSettings):
    ...
    road_snap_strength: float = 0.90
    road_displacement_pull: float = 0.45
    injured_velocity_factor: float = 0.5
    ...
    heatmap_history_decay: float = 0.6
    ...
    drone_clean_decay: float = 0.9
    drone_clean_suppression_strength: float = 0.80
    # Multiplier applied at person-detection cells: prob *= (1 + strength * confidence).
    drone_detection_boost_strength: float = 0.6
    ...
    drone_coverage_radius_m: float = 10.0
    ...
    drone_track_launch_delay_sec: float = 120.0
    # Sequential drone sorties: comma-separated paths (relative to the repo root)
    # played back in order. Each sortie can launch at an independent mission-time
    # second via drone_sortie_launch_delays_sec; when a slot is blank the sortie
    # falls back to launching after the previous flight ends + drone_sortie_gap_sec.
    drone_sortie_files: str = (
        "figure_recognition/results/drone.not_found.JSON,"
        "figure_recognition/results/drone.merged.jsonl"
    )
    # Per-sortie launch time (mission elapsed seconds), comma-separated and
    # aligned with drone_sortie_files. Blank entries use sequential fallback.
    drone_sortie_launch_delays_sec: str = "120,225"
    drone_sortie_gap_sec: float = 10.0
    ...
    # Layer blend weights (0–1): how strongly each layer reshapes the matrix per tick.
    topo_reachability_blend_weight: float = 0.55
    road_layer_blend_weight: float = 0.88
    grid_base_outflow: float = 0.22

    # Tobler/Dijkstra topography (topo_layout parity)
    tobler_flat_speed_kmh: float = 4.2  # hiking speed on flat ground (km/h)
    ...
    @property
    def drone_sortie_launch_delay_list(self) -> list[Optional[float]]:
        raw = self.drone_sortie_launch_delays_sec.strip()
        if not raw:
            return []
